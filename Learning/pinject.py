...
def checksum2(msg):
    ...
    for i in range(0, len(msg), 2):
        ...
        bb = ord(msg[i])
        print(aa,bb)
        w = aa + bb#ord(msg[i]) + (ord(msg[i+1]) << 8)
...
class IP(object):
    def __init__(self, source, destination, payload='', proto=socket.IPPROTO_UDP):
        self.version = 4
...
class UDP(object):
    ...
    def pack(self, src, dst, proto=socket.IPPROTO_UDP):
        ...
        print(1)
        pseudo_header = struct.pack('!4s4sBBH',socket.inet_aton(src), socket.inet_aton(dst), 0, proto, length)
        print(2)
